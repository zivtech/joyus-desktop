...
- _private/docs/recon-operator-setup-runbook.md
tags: []
wp_code: WP07
---
...
Validate that all Recon Operator WPs (01–06) wire together correctly on a clean install, and produce a human-readable runbook for Alex to use when onboarding the operator. This WP is the Phase 1 MVP gate: smoke tests pass, runbook covers every the operator-facing step, and the dogfood DMG is ready to hand off.
...
Create `_private/docs/recon-operator-setup-runbook.md`.

**Audience**: Alex following steps while on a video call with the operator. the operator is non-technical. Every step must be completable without a terminal unless explicitly labeled "Alex runs in terminal."
...
- the operator's Signal contact for credential delivery.

#### Step 1: Install Claude Code (Alex — terminal)
Provide the exact install command from the Claude Code docs. Note: the operator does not need to do this — Alex verifies it is installed before the call, or runs it on the operator's machine via screen share.

Verification: `claude --version` returns a version string.

#### Step 2: Deliver the DMG (Alex)
Options in order of preference:
1. AirDrop from Alex's Mac to the operator's Mac.
2. Shared Google Drive link (time-limited, viewer only).

The operator: double-click the `.dmg` to open it.

#### Step 3: Install the unsigned app (the operator)
The app is not notarized for Phase 1. the operator will see "can't be opened because it is from an unidentified developer."
...
**Wizard Step 2 — Credentials**: the operator enters credentials. Alex delivers each credential via Signal, one at a time. the operator types each value into the correct field and clicks "Save". After all four are saved, the operator clicks "Verify All". All fields should show green check. If any shows red X: confirm the value was copied correctly (watch for trailing spaces).
...
**Wizard Step 3 — Skill file**: Alex copies `~/.claude/skills/joyus-recon.md` from Alex's machine to `~/.claude/skills/` on the operator's machine (via AirDrop or `scp` — confirm path). the operator clicks "Check Again". Should show green check.
...
#### After the Call
Alex: confirm the operator can find the exported zip and knows where to send it (Slack #recon-output or agreed channel).
