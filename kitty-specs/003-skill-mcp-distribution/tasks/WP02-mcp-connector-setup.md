...
- _private/docs/verification/wp02-connector-verification.md
wp_code: WP02
---
...
Configure Anthropic-managed first-party MCP connectors (Atlassian, Slack, Google, Figma, Notion, Playwright) at the org level for both Zivtech and Partner Org Cowork workspaces, enabling all org users to access external tools without local setup.
...
**Target orgs**: Zivtech, Partner Org
...
### T009: Configure connectors for Partner Org org

**Purpose**: Replicate applicable connector setup for Partner Org workspace.

**Steps**:
1. Switch to Partner Org Cowork admin panel.
2. Determine which services Partner Org uses:
   - Atlassian: configure if Partner Org has Jira/Confluence (may not)
   - Slack: configure for Partner Org Slack workspace
   - Google Workspace: configure for Partner Org Google account
3. Configure each applicable connector following the same process as T006-T008.
4. Skip connectors for services Partner Org doesn't use — document which and why.
5. Test each configured connector from a Partner Org user account.

**Validation**: All applicable connectors configured for Partner Org. Skipped connectors documented with rationale.
...
4. Repeat for a Partner Org non-admin user with their org's connectors.
5. Document results: connector name, org, test query, result (pass/fail), notes.

**Files**: `_private/docs/verification/wp02-connector-verification.md`
...
- Partner Org may not use all the same services — only configure what's relevant. Don't force connectors for unused services.

## Done Criteria

- [ ] Atlassian, Slack, Google configured for Zivtech org (T006-T008)
- [ ] Applicable connectors configured for Partner Org org (T009)
...
- **Partner Org service gaps**: Partner Org may not have Atlassian — skip gracefully
